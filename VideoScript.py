...
def GeneratePath(mask, OrigImg, PrevPaths = [], theta = None):
            ...
            if len(center_points_list[0])>1:
                ...
                for i, center_points in enumerate(center_points_list):
                    if i == 0:
                        color = (0, 0, 255)  # Green color for "Ego Path"
                    elif i == 1:
...
def predict (args, video = None, output_name = "out.mp4"):
    ...
    vid = str("C:/Users/Rashid/Desktop/bdd100k/videos/day3.mp4")
    cap = cv2.VideoCapture(vid)
    output_name = "outputday3.mp4"
    ...
    for frame in range(n_frames):
                ...
                #test_pipeline = [cv2.imread(item) for item in cfg_seg.data.test.pipeline]
                #test_pipeline = rep(test_pipeline)
                #data = dict(img_info=dict(filename="file"), img_prefix=None, seg_prefix=None)
                ...
                #modelseg = MMDataParallel(modelseg, device_ids=[0])
                print("1")
                with torch.no_grad():
                    #print("det")
                    outputsdet = modeldet.forward_test([img_tensor], [img_meta], rescale=True)
                ...
                with torch.no_grad():
                    ...
                    #outputsseg = modelseg(return_loss=False, rescale=True, **img_tensor)
                    #print("seg")
                    outputsseg = inference_segmentor(modelseg, img)
                    #outputsseg = modelseg.forward_test([img_tensor], [img_meta], rescale=True)
                    #data = dict(img=img_tensor, img_meta=img_meta)
                    #outputsseg = single_gpu_test(modelseg, [data])
                #print("drawing det")
                #logits = outputsseg[0]['seg_logits'] 
                #print(conf)
                

                #prob_map_class0 = outputsseg[0][0]
                #print(outputsseg)
                #print(det)
                #print("drawing seg")
                seg = Draw_Seg_Mask(outputsseg)
                ...
                comb = CombineMasks(args, seg, det)
                mask1 = cv2.inRange(comb, path_color, path_color)
                comb[mask1==0] = [0, 0, 0]
                #comb = draw(outputsdet, outputsseg, img.shape[0], img.shape[1])
                #print(comb)
                #print("generating")
                #img = find_center_path_smooth(comb,img)
                path, img= GeneratePath(comb, img, paths)
                result = cv2.addWeighted(img, 1, segtemp, 0.5, 0)
                if path: paths.append(path)
                if len(paths)>100: paths.pop(0)
                #find_center_path_smooth(comb, img)
                #cv2.imshow("Result", comb)
                #cv2.waitKey()
                #cv2.destroyAllWindows()

                #mask = prob_map_class0 < 0.7
                #mask = outputsseg < 0.7
                #seg[mask] = 2
